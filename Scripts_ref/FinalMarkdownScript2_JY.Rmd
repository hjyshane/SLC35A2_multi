...
###
```{r}
ptz1hr.counts <- Read10X_h5("~/PTZ_ATAC_scRNA_072024/File/PTZ-1hr/outs/filtered_feature_bc_matrix.h5")
sal1hr.counts <- Read10X_h5("~/PTZ_ATAC_scRNA_072024/File/SAL-1hr/outs/filtered_feature_bc_matrix.h5")
ptz24hr.counts <- Read10X_h5("~/PTZ_ATAC_scRNA_072024/File/PTZ-24hr/outs/filtered_feature_bc_matrix.h5")
sal24hr.counts <- Read10X_h5("~/PTZ_ATAC_scRNA_072024/File/SAL-24hr/outs/filtered_feature_bc_matrix.h5")

ptz1hr.fragpath <- "~/PTZ_ATAC_scRNA_072024/File/PTZ-1hr/outs/atac_fragments.tsv.gz"
sal1hr.fragpath <- "~/PTZ_ATAC_scRNA_072024/File/SAL-1hr/outs/atac_fragments.tsv.gz"
ptz24hr.fragpath <- "~/PTZ_ATAC_scRNA_072024/File/PTZ-24hr/outs/atac_fragments.tsv.gz"
sal24hr.fragpath <- "~/PTZ_ATAC_scRNA_072024/File/SAL-24hr/outs/atac_fragments.tsv.gz"

# Annotation data insert
...
ptz1hr.metadata <- read.csv(
  file = "~/PTZ_ATAC_scRNA_072024/File/PTZ-1hr/outs/per_barcode_metrics.csv",
  header = TRUE,
  row.names = 1
)

sal1hr.metadata <- read.csv(
  file = "~/PTZ_ATAC_scRNA_072024/File/SAL-1hr/outs/per_barcode_metrics.csv",
  header = TRUE,
  row.names = 1
)

ptz24hr.metadata <- read.csv(
  file = "~/PTZ_ATAC_scRNA_072024/File/PTZ-24hr/outs/per_barcode_metrics.csv",
  header = TRUE,
  row.names = 1
)

sal24hr.metadata <- read.csv(
  file = "~/PTZ_ATAC_scRNA_072024/File/SAL-24hr/outs/per_barcode_metrics.csv",
  header = TRUE,
  row.names = 1
)
...
Annotation(ptz1hr) <- annotations 
Annotation(sal1hr) <- annotations 
Annotation(ptz24hr) <- annotations 
Annotation(sal24hr) <- annotations 
...
saveRDS(sal1hr, file = "~/PTZ_ATAC_scRNA_072024/WIP/File/V4seurat/sal1hr_0913.rds")
saveRDS(sal24hr, file = "~/PTZ_ATAC_scRNA_072024/WIP/File/V4seurat/sal24hr_0913.rds")
saveRDS(ptz1hr, file = "~/PTZ_ATAC_scRNA_072024/WIP/File/V4seurat/ptz1hr_0913.rds")
saveRDS(ptz24hr, file = "~/PTZ_ATAC_scRNA_072024/WIP/File/V4seurat/ptz24hr_0913.rds")
...
saveRDS(combo.all, file = '~/PTZ_ATAC_scRNA_072024/WIP/File/V4seurat/combo.allRNAintegration_0913.rds')
...
saveRDS(sal1hr, file = "~/PTZ_ATAC_scRNA_072024/WIP/File/V4seurat/sal1hr_atac_0913.rds")
saveRDS(sal24hr, file = "~/PTZ_ATAC_scRNA_072024/WIP/File/V4seurat/sal24hr_atac_0913.rds")
saveRDS(ptz1hr, file = "~/PTZ_ATAC_scRNA_072024/WIP/File/V4seurat/ptz1hr_atac_0913.rds")
saveRDS(ptz24hr, file = "~/PTZ_ATAC_scRNA_072024/WIP/File/V4seurat/ptz24hr_atac_0913.rds")
...
atac.integration.anchors <- FindIntegrationAnchors(
  ...
  anchor.features = rownames(ptz1hr),  # Features from the ATAC assay
  reduction = "rlsi",  # Reciprocal LSI for ATAC data
  dims = 2:30
)
...
saveRDS(combo.all, file = '~/PTZ_ATAC_scRNA_072024/WIP/File/V4seurat/combo.allATACintegration.rds')
...
write.csv(combo.all.markers, file="~/PTZ_ATAC_scRNA_072024/WIP/File/V4seurat/combo.all.markers.csv")
...
mouseatlas.cut <- readRDS("/igm/home/hxy008/PTZ_ATAC_scRNA_072024/WIP/File/mouseatlas_cut_processed.rds")
library(future)
options(future.globals.maxSize = 16000 * 1024^2)
mouseatlas.cut <-SCTransform(mouseatlas.cut, verbose = FALSE)

#
DefaultAssay(combo.all) <- "SCT"
#
anchors <- FindTransferAnchors(reference = mouseatlas.cut,
                               query = combo.all,
                               dims = 1:30,
                               normalization.method = "SCT")
#
predictions <- TransferData(anchorset = anchors,
                            refdata = mouseatlas.cut$Description,
                            dims = 1:30)
combo.all <- AddMetaData(object = combo.all, metadata = predictions)

#
combo.all$cell_types <- combo.all$predicted.id
Idents(combo.all) <- combo.all$cell_types
unique(combo.all$cell_types)
...
long_names <- c(
  ...
  "Vasucular endothelial cells, capillary",
  "Inhibitory interneurons, hippocampus",
  ...
  "Vasucular endothelial cells, venous",
  "Pericytes"
  )
...
#
combo.all$celltype_short <- shorten_names(combo.all$cell_types)
Idents(combo.all) <- combo.all$celltype_short
Idents(combo.all) <- combo.all$seurat_clusters
DimPlot(combo.all, reduction = "umap.harmony", label = T, repel = T) + NoAxes() + NoLegend()
saveRDS(combo.all, file = '~/PTZ_ATAC_scRNA_072024/WIP/File/V4seurat/combo.allAnnotation.rds')
```
