...
deg_wide <- deg_wide %>%
  mutate(
    pattern = case_when(
      PTZvsSAL_1hr > 0 & PTZvsSAL_24hr > 0 ~ "Sustained Increase",
...
pattern_celltype_counts <- deg_wide %>%
  count(pattern, cell_type)

bar_celltype <- pattern_celltype_counts %>%
  ggplot(aes(x = reorder(pattern, -n, sum), y = n, fill = cell_type)) +
  geom_bar(stat = "identity", position = "stack") +  # proportion fill
  coord_flip() +
  scale_fill_brewer(palette = "Set3") +
  theme_minimal() +
  labs(
    title = "Proportion of Cell Types per Expression Pattern",
    x = "Expression Pattern", y = "Proportion",
    fill = "Cell Type"
  )
ggsave(filename = file.path(output_dir_base, "DEG", "Plots", "Gene_Behavior", "BarPlot_all_by_pattern_cellstack.png"), bar_celltype, width = 16, height =12, dpi = 300)
...
# Define path to CSVs
csv_files <- list.files("~/PTZ_ATAC_scRNA_072024/WIP/0624_run/DEG/sig_csv/", pattern = ".csv$", full.names = TRUE)
...
for (file in csv_files) {
  deg <- read.csv(file)
  if (nrow(deg) == 0) next
  conversion <- clusterProfiler::bitr(deg$gene, fromType = "SYMBOL", toType = "ENTREZID", OrgDb = org.Mm.eg.db)
...
saveRDS(kegg_results, "~/PTZ_ATAC_scRNA_072024/WIP/0624_run/KEGG/kegg_results.rds")
...
for (dataset in names(kegg_results)) {
  ...
  if (!is.null(up_reg) && nrow(up_reg) > 0) {
    ...
    ggsave(filename = file.path(base_dir, "KEGG", paste0(dataset, "_KEGG_up.png")), plot = up, width = 12, height = 12, dpi = 300)
    }
  
  if (!is.null(down_reg) && nrow(down_reg) > 0) {
    down <- dotplot(kegg_results[[dataset]]$down, showCategory = 15) +
    ggtitle(paste("KEGG_Down - ", dataset))
    ggsave(filename = file.path(base_dir, "KEGG", paste0(dataset, "_KEGG_down.png")), plot = down, width = 12, height = 12, dpi = 300)
    }
}
...
write_csv(kegg_summary_df,"~/PTZ_ATAC_scRNA_072024/WIP/0624_run/KEGG/kegg_summary.csv")
...
# Barplot
output_dir <- file.path(base_dir, "KEGG", "Barplots")
