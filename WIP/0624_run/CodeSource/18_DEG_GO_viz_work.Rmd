...
deg_dir <- file.path(base_dir, "DEG_MAST", "bg_csv")
...
deg_list <- list.files(deg_dir, full.names = T, pattern = "^ExN_CA|^ExN_mDG")
go_bp_list <- list.files(go_bp_dir, full.names = T, pattern = "^GO_BP_ExN_CA|^GO_BP_ExN_mDG")
go_mf_list <- list.files(go_mf_dir, full.names = T, pattern = "^GO_MF_ExN_CA|^GO_MF_ExN_mDG")
go_cc_list <- list.files(go_cc_dir, full.names = T, pattern = "^GO_CC_ExN_CA|^GO_CC_ExN_mDG")
...
for (i in seq_along(temp_bp)) {
  bp_gene <- temp_bp[[i]] %>%
    ...
    rename(gene = geneID, 
           ...
           GO_ct = cell_type,
           GO_cp = comparison) %>%
    dplyr::select(ID, GO, gene, GO_ont, GO_p, GO_q, GO_fc, GO_r, GO_ont, GO_ct, GO_cp)
...
final <- all %>%
  ...
  rename(DEG_p = p_val,
         ...
         DEG_sig = Viz,
         DEG_edit = gene_edit) %>% 
  relocate(gene, GO, GO_ont, GO_q, DEG_adjp, GO_fc, DEG_log2FC, everything())

# Save
write_csv(final, file = file.path(deg_dir, "combined_DEG_GO_gene_list.csv"))
...
for (ont in c("BP", "MF", "CC")) {
  for (ct in cell) {
    for (cp in comp){
      ...
      if (nrow(temp_df) == 0) {
        cat("Skipping empty dataset:", ont, ct, cp, "\n")
        next
      }
