...
plan("multicore", workers = 16)  # Use multiple cores for parallel processing
...
library(dplyr)       # Data manipulation (filter, mutate, select, group_by, etc.)
library(tidyr)       # Data tidying (pivot, separate, unite, fill, etc.)
library(plyr)        # Older version of dplyr (not always needed)
...
library(tibble)      # Modern data frame representation
...
library(GSEA)             # Gene Set Enrichment Analysis
...
# File Handling & Misc
library(readr)  # CSV/TSV file reading
...
output_dir_base <-"~/PTZ_ATAC_scRNA_072024/WIP/0428run"

# Set base input folder directory files.
input_dir <- "~/PTZ_ATAC_scRNA_072024/File"

dir.create("~/PTZ_ATAC_scRNA_072024/WIP/0428run/RDS_mid", recursive = TRUE, showWarnings = FALSE)
```
...
# Integrate data & save for mid point
# Combines all datasets into a single integrated Seurat object.
# Uses the identified anchors to align and merge the datasets, correcting for technical variation while retaining biological variation.
# The integrated data is stored in the "integrated" assay of the new Seurat object.
...
marker_results <- list()
for (i in seq(0.4, 1.2, by = 0.1)) {
  Idents(integrated_rna) <- integrated_rna[[paste0('RNA_cluster_', i)]]
...
dir.create("~/PTZ_ATAC_scRNA_072024/WIP/0428run/csv", recursive = TRUE, showWarnings = FALSE)

for (name in names(marker_results)) {
  write.csv(marker_results[[name]], file = paste0("~/PTZ_ATAC_scRNA_072024/WIP/0428run/csv", name, "_markers.csv"), row.names = FALSE)
}
```
...
# Claer workspace
# rm("mm10", "annotations", "peak_beds", "frag_paths")
```
...
# Cluster
for (i in seq(0.4, 1.2, by = 0.1)) {
  merged_atac <- FindClusters(merged_atac, graph.name = "RNA_snn", resolution = i, verbose = T, cluster.name = paste0("ATAC_cluster_", i))
}
```
...
for (i in seq(0.4, 1.2, by = 0.1)) {
  Idents(merged_atac) <- merged_atac[[paste0('ATAC_cluster_', i)]]
...
for (name in names(marker_results_atac)) {
  write.csv(marker_results_atac[[name]], file = paste0(name, "_markers.csv"), row.names = FALSE)
}
```
...
for (i in seq(0.4, 1.2, by = 0.1)) {
  Idents(combined_merged) <- combined_merged[[paste0('ATAC_integrated_cluster_', i)]]
  ...
  Idents(combined_merged) <- combined_merged[[paste0('RNA_integrated_cluster_', i)]]
...
for (name in names(marker_results_combined)) {
  write.csv(marker_results_combined[[name]], file = paste0(name, "_markers.csv"), row.names = FALSE)
}
```
...
combined_merged <- FindMultiModalNeighbors(object = combined_merged, 
                                           ...
                                           modality.weight.name = "Multimodal.weight", # modality.weight.name: Adds weights to indicate the relative contribution of RNA and ATAC.
                                           verbose = TRUE)
...
for (i in seq(0.4, 1.2, by = 0.1)) {
  Idents(combined_merged) <- combined_merged[[paste0('Multimodal_cluster_', i)]]
  marker_results_multi[[paste0("Multimodal_cluster_", i)]] <- FindAllMarkers(combined_merged, 
                                                                ...
                                                                min.pct = 0.25, 
                                                                logfc.threshold = 0.25,
                                                                graph.name = "Multimodal_wsnn")
}
```

```{r Save point - multimodal markers, echo=TRUE, error=TRUE, message=TRUE, warning=TRUE}
for (name in names(marker_results_multi)) {
  write.csv(marker_results_multi[[name]], file = paste0("~/PTZ_ATAC_scRNA_072024/WIP/0428run/csv", name, "_markers.csv"), row.names = FALSE)
}
```
...
ggsave(qc_feature_combined, filename = file.path(output_dir_base, "QC", "Plots", "qc_featureplot.png"), width = 8, height = 8, dpi = 300)
...
```{r Link ATAC and RNA for coverage plot , echo=TRUE, error=TRUE, message=TRUE, warning=TRUE}
...
multimodal_cell_type_mapping <- c(
  "1" = "ExcitatoryNeuronsMatureDG", "11" = "ExcitatoryNeuronsMatureDG",
  ...
  "12" = "L23ExcitatoryNeurons", "19" = "L23ExcitatoryNeurons",  
  
  "15" = "L23ExcitatoryNeurons", 
  
  "10" = "L45ExcitatoryNeurons",  "17" = "L45ExcitatoryNeurons", "14" = "L45ExcitatoryNeurons",

  "5" = "L56ExcitatoryNeurons", "24" = "L56ExcitatoryNeurons", 
  
  "3" = "ExcitatoryNeuronsCA3", "18" = "ExcitatoryNeuronsCA3", 

  "0" = "ExcitatoryNeuronsCA1", "8" = "ExcitatoryNeuronsCA1", "13" = "ExcitatoryNeuronsCA1", 

  "20" = "NdnfRelnInhibitoryInterneurons")
...
create_feature_plot <- function(seurat_obj, gene, filename) {
  ...
  ggsave(plot, filename = paste0("~/PTZ_ATAC_scRNA_072024/WIP/Finalized/Plots/FeaturePlots/", "FeaturePlot_", gene, "_multi.png"), width = 8, height = 7, dpi = 300)}
...
create_vln_plot <- function(seurat_obj, gene, filename) {
  plot <- VlnPlot(seurat_obj, features = gene,
    ...
    theme_minimal() +
    theme(legend.position = "right")
  ggsave(plot, filename = paste0("~/PTZ_ATAC_scRNA_072024/WIP/Finalized/Plots/VlnPlots/", "VlnPlot_", gene, "_multi.png"), width = 8, height = 7, dpi = 300)}
```
...
ggsave(filename = file.path(output_dir_base, "Plots", "DimPlots", "Final", "Multi_UMAP_timepoint.png"), width = 12, height = 12, dpi = 300, limitsize = F)
...
comparisons_to_analyze <- list(list(name = "PTZvsSAL_24hr", group1 = "PTZ_24hr", group2 = "SAL_24hr"),
                               list(name = "PTZvsSAL_1hr", group1 = "PTZ_1hr", group2 = "SAL_1hr"),
                               list(name = "24hrvs1hr_PTZ", group1 = "PTZ_24hr", group2 = "PTZ_1hr"),
                               list(name = "24hrvs1hr_SAL", group1 = "SAL_24hr", group2 = "SAL_1hr"))

```

```{r DEG loop, echo=TRUE, error=TRUE, message=TRUE, warning=TRUE}
output_dir_base <- "~/PTZ_ATAC_scRNA_072024/WIP/Finalized"
  ...
  for (comp in comparisons_to_analyze) {
      ...
      if (cell_count_group1 >= 3 && cell_count_group2 >= 3) {
        ...
        de_results <- FindMarkers(
          ...
          min.pct = 0.25,
          logfc.threshold = 0.0,
          return.thresh = 1.01, # not used by MAST test.
          test.use = "wilcox"
        )
        ...
        csv_filename <- sprintf("DE_results_%s_%s.csv", cell_type, safe_comp_name)
        write.csv(
          de_results,
          file = file.path(output_dir_base, "Filtered_50_cells", "DE_gene_MAST", csv_filename),
          row.names = FALSE
        )
...
# file read
csv_files <- list.files(file.path(output_dir_base,"Filtered_50_cells", "DE_gene_wilcox"), pattern = "DE_results_.+\\.csv$", full.names = TRUE)
...
for (csv_file in csv_files) {
  ...
  if (nrow(de_results) == 0) {
    ...
    ggsave(file.path(output_dir_base, "Filtered_50_cells", "Plots","VolcanoPlots_wilcox", plot_file_name), volcano_plot, width = 12, height = 10)
...
for (csv_file in csv_files) {
  ...
  write.csv(significant_genes, file = file.path(output_dir_base, "Filtered_50_cells", "Sig_gene_wilcox", sig_genes_filename), row.names = FALSE)
...
create_deg_summary <- function(deg_dir) {
  ...
  for (file in deg_files) {
    ...
    up_regulated <- sum(deg_data$avg_log2FC > 0.5 & deg_data$p_val_adj < 0.05)
    down_regulated <- sum(deg_data$avg_log2FC < -0.5 & deg_data$p_val_adj < 0.05)
    significant_genes <- sum(deg_data$p_val_adj < 0.05)
    
    # Compute top genes
    top_up_genes <- deg_data$gene[deg_data$avg_log2FC > 0.5 & deg_data$p_val_adj < 0.05]
    top_down_genes <- deg_data$gene[deg_data$avg_log2FC < -0.5 & deg_data$p_val_adj < 0.05]
...
# Save summary
write.csv(complete_summary, file.path(output_dir_base,  "DEG_Complete_Analysis_Summary"), row.names = FALSE)
```

```{r DEG comparison, echo=TRUE, error=TRUE, message=TRUE, warning=TRUE}
# Read the summary files
complete_summary <-read.csv("~/PTZ_ATAC_scRNA_072024/WIP/ModuleTest/DEG_Complete_Analysis_Summary_wilcox.csv")
...
ggsave(file.path("~/PTZ_ATAC_scRNA_072024/WIP/ModuleTest/Sig_UpDown_Distribution_stacked_wilcox.png"), p1_stacked, width = 18, height = 12)
ggsave(file.path("~/PTZ_ATAC_scRNA_072024/WIP/ModuleTest/Sig_UpDown_Distribution_wilcox.png"), p1, width = 16, height = 12)
...
ggsave(file.path("~/PTZ_ATAC_scRNA_072024/WIP/ModuleTest/DEG_vs_SigGenes_wilcox.png"), p2, width = 16, height = 12)
ggsave(file.path("~/PTZ_ATAC_scRNA_072024/WIP/ModuleTest/DEG_vs_SigGenes_stacked_wilcox.png"), p2_stacked, width = 16, height = 12)
...
for (comp in unique(deg_long$Comparison)) {
  ...
  ggsave(file.path(paste0("~/PTZ_ATAC_scRNA_072024/WIP/ModuleTest/Summary/Sig_UpDown_", comp, "_from_summary_wilcox.png")), p4, width = 10, height = 6, dpi = 300)
}
...
for (comp in unique(long_summary$Comparison)) {
  ...
  ggsave(file.path(paste0("~/PTZ_ATAC_scRNA_072024/WIP/ModuleTest/Summary/DEG_vs_Sig_", comp, "_from_summary_wilcox.png")), p5, width = 10, height = 6, dpi = 300)
}
...
for (comp in unique(deg_long$Comparison)) {
  ...
  ggsave(file.path(paste0("~/PTZ_ATAC_scRNA_072024/WIP/ModuleTest/Summary/Sig_UpDown_", comp, "_stacked_wilcox.png")), 
...
for (comp in unique(long_summary$Comparison)) {
  ...
  ggsave(file.path(paste0("~/PTZ_ATAC_scRNA_072024/WIP/ModuleTest/Summary/DEG_vs_Sig_", comp, "_stacked_wilcox.png")), 
...
# Read the summary files
complete_summary <- read.csv(file.path(output_dir_base, "Filtered_50_cells", "Summary", "DEG_Complete_Analysis_Summary_MAST.csv"))
...
for (comp in comparisons) {
  ...
  ggsave(filename = file.path(output_dir_base,  "Plots", "ScatterPlots", plot_file_name), plot = scatter_plot, width = 10, height = 8, dpi = 300)
}
...
write.csv(correlation_results_df, file = file.path(output_dir_base, "Correlation_Results_wilcox.csv"), row.names = FALSE)
```
...
# Get list of files
input_dir <- "~/PTZ_ATAC_scRNA_072024/WIP/Finalized/Filtered_50_cells/Sig_gene_wilcox"
...
output_dir_base <- "~/PTZ_ATAC_scRNA_072024/WIP/Finalized/Filtered_50_cells/GO_Analysis_bgfix_wilcox"
...
# dir set
input_dir <- "~/PTZ_ATAC_scRNA_072024/WIP/Finalized/"

# File path and list of files
sig_files <- list.files(file.path(input_dir, "Filtered_50_cells", "Sig_gene"), full.names = T)

output_dir_base <- "~/PTZ_ATAC_scRNA_072024/WIP/Finalized/Filtered_50_cells/GO_Analysis_bgfix"
output_dir_csv <- file.path(output_dir_base, "csv/")
output_dir_rds <- file.path(output_dir_base, "enrichRobject/")
output_dir_dot <- file.path(output_dir_base, "GO_plot/DotPlot/")
output_dir_cnetplot <- file.path(output_dir_base, "GO_plot/Cnetplots/")
...
combined_merged <- readRDS("~/PTZ_ATAC_scRNA_072024/WIP/Finalized/RDS_mid/combined_merged_atac_work.rds")
...
count_gene_go_terms <- function(input_dir, output_dir_plot, output_dir_csv, output_file) {
  ...
  go_files <- list.files(input_dir, pattern = "GO_results_.+\\.csv$", full.names = TRUE)
  ...
  for (file in go_files) {
    ...
    condition <- gsub("GO_results_|.csv", "", basename(file))
...
count_gene_top20_with_report <- function(input_dir, output_dir_plot, output_dir_csv, output_file) {
  ...
  write_csv(all_counts_top, file.path(input_dir, "gene_go_term_counts_top20.csv"))
...
go_csv <- "~/PTZ_ATAC_scRNA_072024/WIP//GO_plot/csv/"
output_dir_gf_csv <- "~/PTZ_ATAC_scRNA_072024/WIP/ModuleTest/GO_Analysis/Gene_frequency/summary/"
output_dir_gf_plot <- "~/PTZ_ATAC_scRNA_072024/WIP//ModuleTest/GO_Analysis/Gene_frequency/plots/"
output_file <- "~/PTZ_ATAC_scRNA_072024/WIP/ModuleTest/GO_Analysis/Gene_frequency/summary/gene_combined.csv"
output_file_top20 <- "~/PTZ_ATAC_scRNA_072024/WIP/ModuleTest/GO_Analysis/Gene_frequency/summary/gene_combined_top20.csv"
...
# Set directories
input_dir <- "~/PTZ_ATAC_scRNA_072024/WIP/Finalized/Filtered_50_cells/GO_Analysis/Gene_frequency/summary/"
output_dir_euler <- "~/PTZ_ATAC_scRNA_072024/WIP/Finalized/Filtered_50_cells/GO_Analysis/GO_plot/Euler/"
...
# Define path to CSVs
csv_files <- list.files("~/PTZ_ATAC_scRNA_072024/WIP/Finalized/Filtered_50_cells/Sig_gene/", pattern = "Significant_genes_.*\\.csv$", full.names = TRUE)

# Load all CSVs into a list
deg_list <- list()

# add entrezid and keep only more than 10 gene list
for (file in csv_files[9:36]) {
  deg <- read.csv(file)
  if (nrow(deg) == 0) next
  ...
  if (nrow(deg) == 0) next
  deg <- select(deg, gene, ENTREZID, Regulation, cell_type, comparison)
  up_count <-  sum(deg$Regulation == "Upregulated")
  down_count <- sum(deg$Regulation == "Downregulated")
  
  if (up_count >= 10 | down_count >=10) {
  clean_name <- gsub("Significant_genes_|.csv", "", basename(file))  # Clean file name
...
kegg_results <- lapply(deg_list, function(df) {
  ...
  up_genes <- df$ENTREZID[df$Regulation == "Upregulated"]
  down_genes <- df$ENTREZID[df$Regulation == "Downregulated"]
...
saveRDS(kegg_results, "~/PTZ_ATAC_scRNA_072024/WIP/Finalized/Filtered_50_cells/KEGG/kegg_results.rds")
...
for (dataset in names(kegg_results)) {
  ...
  if (!is.null(up_reg) && nrow(up_reg) > 0) {
    ...
    ggsave(filename = file.path(output_dir_base, "Filtered_50_cells", "KEGG", paste0(dataset, "_KEGG_up.png")), plot = up, width = 12, height = 12, dpi = 300)
    }
  
  if (!is.null(down_reg) && nrow(down_reg) > 0) {
    down <- dotplot(kegg_results[[dataset]]$down, showCategory = 15) +
    ggtitle(paste("KEGG_Down - ", dataset))
    ggsave(filename = file.path(output_dir_base, "Filtered_50_cells", "KEGG", paste0(dataset, "_KEGG_down.png")), plot = down, width = 12, height = 12, dpi = 300)
    }
}

# Extract KEGG p-values for each dataset
kegg_summary <- lapply(kegg_results, function(res) {
  cell_type <- as.character(unique(res$cell_type))  # Extract cell type
  comparison <- as.character(unique(res$comparison))  # Extract comparison name
  up_res <- res$up@result %>% select(Description, p.adjust) %>% mutate(Regulation = "Upregulated", CellType = cell_type, Comparison = comparison)
  down_res <- res$down@result %>% select(Description, p.adjust) %>% mutate(Regulation = "Downregulated", CellType = cell_type, Comparison = comparison)
  bind_rows(up_res, down_res)  # Combine up/down pathways
})
...
# Barplot
output_dir <- file.path(output_dir_base, "Filtered_50_cells", "KEGG", "KEGG_Barplots")
...
for (comparison in unique(top_kegg$Comparison)) {
  ...
  p <- ggplot(subset_data, aes(x = reorder(Description, -p.adjust), y = -log10(p.adjust), fill = CellType)) +
    ...
    scale_x_discrete(labels = wrap_format(30))  # Wrap long pathway names
...
```{r fGSEA analysis setup, echo=TRUE, error=TRUE, message=TRUE, warning=TRUE}
# Define directory where DEG results are stored
deg_dir <- "~/PTZ_ATAC_scRNA_072024/WIP/Finalized/Filtered_50_cells/DE_gene"

# List all CSV files in the directory
deg_files <- list.files(deg_dir, pattern = "DE_results_.*\\.csv", full.names = TRUE)

# Function to safely read CSVs and fix column types
read_and_standardize_csv <- function(file) {
  df <- {
    # Read CSV using readr::read_csv to avoid conflicts
    data <- readr::read_csv(file, col_types = cols(.default = "c"))  # Read everything as character first

    # Ensure expected columns exist
    required_cols <- c("gene", "avg_log2FC", "p_val_adj", "cell_type", "comparison")
...
# Read and standardize all CSV files
deg_results_list <- lapply(deg_files[9:36], read_and_standardize_csv)

# Combine all files
deg_results <- bind_rows(deg_results_list)
```

```{r Ranked list prep, echo=TRUE, error=TRUE, message=TRUE, warning=TRUE}
# Function to create ranked lists for each cell type and comparison
create_ranked_list <- function(df, cell_type_name, comparison_name) {
  ranked_df <- df %>%
    filter(cell_type == cell_type_name, comparison == comparison_name) %>%
    mutate(rank_metric = avg_log2FC * -log10(p_val_adj)) %>%
    arrange(desc(rank_metric))
  
  # Remove duplicate gene names, keeping the highest absolute rank
  ranked_df <- ranked_df %>%
    group_by(gene) %>%
  ...
  ranked_list <- setNames(ranked_df$rank_metric, ranked_df$gene)

  return(ranked_list)
}

# Get unique cell types & comparisons
unique_celltypes <- unique(deg_results$cell_type)
unique_comparisons <- unique(deg_results$comparison)

# Generate ranked lists for each (CellType + Comparison) pair
ranked_gene_lists <- list()

for (cell in unique_celltypes) {
  for (comp in unique_comparisons) {
    ranked_list <- create_ranked_list(deg_results, cell, comp)
    if (length(ranked_list) > 0) {  # Only store non-empty lists
      ranked_gene_lists[[paste(cell, comp, sep = "_")]] <- ranked_list
    }
  }
}

# Function to convert gene symbols to ENTREZ IDs
convert_to_entrez <- function(ranked_list) {
  # Convert gene symbols to ENTREZ
  conversion <- bitr(names(ranked_list), fromType = "SYMBOL", toType = "ENTREZID", OrgDb = org.Mm.eg.db)
  
  # Match ENTREZ IDs to original ranked list
  converted_ranked_list <- ranked_list[names(ranked_list) %in% conversion$SYMBOL]
  names(converted_ranked_list) <- conversion$ENTREZID[match(names(converted_ranked_list), conversion$SYMBOL)]
  
  # Remove any NA values after conversion
  converted_ranked_list <- na.omit(converted_ranked_list)
  
  return(converted_ranked_list)
}

# Apply conversion to all ranked gene lists
ranked_gene_lists_entrez <- lapply(ranked_gene_lists, convert_to_entrez)
```

```{r fgsea, echo=TRUE, error=TRUE, message=TRUE, warning=TRUE}
# Load KEGG pathways
fgsea_pathways <- msigdbr(species = "Mus musculus", category = "C2", subcategory = "KEGG")

# Convert KEGG pathways into fgsea format
fgsea_list <- split(fgsea_pathways$entrez_gene, fgsea_pathways$gs_name)

# Run GSEA and assign names dynamically
fgsea_results_list <- setNames(
  lapply(names(ranked_gene_lists_entrez), function(comp) {
    message(sprintf("Running GSEA for %s", comp))
    
    # Run GSEA
    fgsea_res <- fgseaMultilevel(pathways = fgsea_list, stats = ranked_gene_lists_entrez[[comp]])
    
    # Add metadata
    fgsea_res$comparison <- comp  
    return(fgsea_res)
  }), 
  names(ranked_gene_lists_entrez)  # Assign list names
)
...
fgsea_results_filtered <- fgsea_results %>% filter(padj < 0.05) %>% arrange(NES)

# visualize
# get cell and condition type and add.
fgsea_results_filtered <- fgsea_results_filtered %>%
  mutate(
    cell_type = sapply(strsplit(comparison, "_"), `[`, 1),
    condition = sapply(strsplit(comparison, "_"), function(x) paste(x[-1], collapse = "_"))
  )

# function fro plot.
plot_dot_for_cell <- function(cell_type_name, data) {
  # Subset data for the selected cell type
  subset_data <- data %>% filter(cell_type == cell_type_name)
  
  # Create the dot plot
  p <- ggplot(subset_data, aes(x = condition, y = pathway)) +
  geom_point(aes(size = abs(NES), color = NES)) +
  scale_color_gradient2(low = "blue", mid = "white", high = "red", midpoint = 0) +
  labs(title = paste("Enriched KEGG Pathways for", cell_type_name),
       x = "Condition",
       y = "KEGG Pathway",
       size = "Absolute NES",
       color = "NES") +
...
unique_cell_types <- unique(fgsea_results_filtered$cell_type)
output_dir <- "~/PTZ_ATAC_scRNA_072024/WIP/Finalized/Filtered_50_cells/fGSEA"

# Loop over cell types and generate plots
for (ct in unique_cell_types) {
  # Generate the plot using your custom function
  p <- plot_dot_for_cell(ct, fgsea_results_filtered)
  
  # Define a file name for each cell type plot
  file_name <- file.path(output_dir, paste0(ct, "_fGSEA_dotplot.png"))
...
```{r subset object, echo=TRUE, error=TRUE, message=TRUE, warning=TRUE}
...
# save
ggsave(filename = "~/PTZ_ATAC_scRNA_072024/WIP/Finalized/Filtered_50_cells/Trajectory/combined.png", plot = pseudo, width = 12, height = 12, dpi = 300)
ggsave(filename = "~/PTZ_ATAC_scRNA_072024/WIP/Finalized/Filtered_50_cells/Trajectory/grouped.png", plot = condition, width = 12, height = 12, dpi = 300)
ggsave(filename = "~/PTZ_ATAC_scRNA_072024/WIP/Finalized/Filtered_50_cells/Trajectory/combined_ca.png", plot = pseudo_ca, width = 12, height = 12, dpi = 300)
ggsave(filename = "~/PTZ_ATAC_scRNA_072024/WIP/Finalized/Filtered_50_cells/Trajectory/grouped_ca.png", plot = condition_ca, width = 12, height = 12, dpi = 300)
ggsave(filename = "~/PTZ_ATAC_scRNA_072024/WIP/Finalized/Filtered_50_cells/Trajectory/combined_dg.png", plot = pseudo_dg, width = 12, height = 12, dpi = 300)
ggsave(filename = "~/PTZ_ATAC_scRNA_072024/WIP/Finalized/Filtered_50_cells/Trajectory/grouped_dg.png", plot = condition_dg, width = 12, height = 12, dpi = 300)
...
# save
ggsave(filename = "~/PTZ_ATAC_scRNA_072024/WIP/Finalized/Filtered_50_cells/Trajectory/density.png", plot = density, width = 12, height = 12, dpi = 300)
ggsave(filename = "~/PTZ_ATAC_scRNA_072024/WIP/Finalized/Filtered_50_cells/Trajectory/bar.png", plot = bar, width = 12, height = 12, dpi = 300)
ggsave(filename = "~/PTZ_ATAC_scRNA_072024/WIP/Finalized/Filtered_50_cells/Trajectory/density_ca.png", plot = density_ca, width = 12, height = 12, dpi = 300)
ggsave(filename = "~/PTZ_ATAC_scRNA_072024/WIP/Finalized/Filtered_50_cells/Trajectory/bar_ca.png", plot = bar_ca, width = 12, height = 12, dpi = 300)
ggsave(filename = "~/PTZ_ATAC_scRNA_072024/WIP/Finalized/Filtered_50_cells/Trajectory/density_dg.png", plot = density_dg, width = 12, height = 12, dpi = 300)
ggsave(filename = "~/PTZ_ATAC_scRNA_072024/WIP/Finalized/Filtered_50_cells/Trajectory/bar_dg.png", plot = bar_dg, width = 12, height = 12, dpi = 300)
...
saveRDS(combined_merged, "~/PTZ_ATAC_scRNA_072024/WIP/Finalized/RDS_mid/combined_merged_atac_work.rds")
```
...
comparisons_to_analyze <- list(
  list(name = "PTZvsSAL_24hr", group1 = "PTZ_24hr", group2 = "SAL_24hr"),
  list(name = "PTZvsSAL_1hr", group1 = "PTZ_1hr", group2 = "SAL_1hr"),
  list(name = "24hrvs1hr_PTZ", group1 = "PTZ_24hr", group2 = "PTZ_1hr"),
  list(name = "24hrvs1hr_SAL", group1 = "SAL_24hr", group2 = "SAL_1hr")
)
...
# output path
output_dir_base <-"~/PTZ_ATAC_scRNA_072024/WIP/Finalized/Filtered_50_cells/"
  ...
  for (comp in comparisons_to_analyze) {
    tryCatch({
      ...
      if (cell_count_group1 >= 3 && cell_count_group2 >= 3) {
        ...
        # **✅ NEW: Perform Motif Analysis on Significant DA Peaks**
...
```
``` {r,}
motif_up <- read_csv('~/PTZ_ATAC_scRNA_072024/WIP/Finalized/Filtered_50_cells/Motif_Enrichment/Motif_results_ExcitatoryNeuronsCA1_24hrvs1hr_PTZ_DOWN.csv')

motif_up$logpadu <- -log10(motif_up$p.adjust) 
motif_up <- motif_up %>% filter(p.adjust != 1) 
top_motifs <- motif_up[order(motif_up$p.adjust), ][1:10, ]

ggplot(motif_up, aes(x = reorder(motif, logpadu), y = logpadu)) +
  geom_point(aes(size = fold.enrichment, color = p.adjust)) +
  scale_color_viridis_c(option = "plasma", direction = -1) +
  coord_flip() +
  labs(title = "Enriched Motifs (Upregulated Peaks)",
       x = "Motif",
       y = "-log10(adjusted p-value)",
...
ggplot(top_motifs, aes(x = reorder(top_motifs, logpadu), y = logpadu)) +
  geom_bar(stat = "identity", fill = "steelblue") +
  coord_flip() +
  labs(title = "Top 10 Enriched Motifs (Upregulated Peaks)",
       x = "Motif",
       y = "-log10(adjusted p-value)") +
  theme_minimal()
```
...
write.csv(final_candidate_df, '~/PTZ_ATAC_scRNA_072024/WIP/Finalized/Filtered_50_cells/GO_Analysis/gene_go_list.csv')

```

```{r DAG volcano plot, echo=TRUE, error=TRUE, message=TRUE, warning=TRUE}
# file read
csv_files <- list.files(file.path(output_dir_base, "Filtered_50_cells" ,"DA_ATAC"), pattern = "DA_results_.+\\.csv$", full.names = TRUE)
